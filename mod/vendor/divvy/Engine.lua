...
-- Modified for Yubalatro on 2026-09-08; see NOTICE.md.
...
if not DV.SIM.run then
  ...
  function DV.SIM.init()
     ...
     DV.SIM.env.poker_hands = poker_hands -- Yubalatro: required by boss hand checks.
  ...
  function DV.SIM.get_card_data(card_obj)
     return {
        sort_id = card_obj.sort_id, -- Yubalatro: preserve Hook's selection ordering.
  ...
  function DV.SIM.simulate_joker_global_effects()
     for _, joker in ipairs(DV.SIM.env.jokers) do
        if joker.edition and not joker.debuff then -- Foil and Holo:
        ...
        if joker.edition and not joker.debuff then -- Poly:
  ...
  function DV.SIM.simulate_card_in_context(card, cardarea)
     if card.debuff and cardarea == G.play then return end -- Yubalatro: no played-card triggers.
