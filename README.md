...
| 每回合弃牌次数 | 0–99 | 每个盲注开始时可弃牌的次数，可设为 0 |
| 初始金钱 | 0–999999999 | 新开局拥有的钱 |

例如：手牌上限 `12`、出牌次数 `8`、弃牌次数 `6`、初始金钱 `1000`。

设置页显示已保存的数值；未自定义的项目会用灰色数字显示当前牌组的原版开局值，例如红色牌组的手牌、出牌、弃牌、金钱分别为 `8`、`4`、`4`、`4`。加减按钮从显示的数值开始调整，并自动限制在允许范围内。旧配置没有弃牌次数时沿用原版规则。
各项留空时保留原版数值。首次安装全部留空，点击「恢复原版设置」可清空所有覆盖值并重新显示原版开局值。
填写的数值在牌组和挑战的开局规则处理后应用，例如黄色牌组填写 `1000` 后开局为 `$1000`。
开局后的小丑牌、优惠券、Boss 等效果仍可改变手牌上限、出牌和弃牌次数。手牌上限也受剩余可抽牌数量限制。
一次最多选择五张牌的原版规则保持不变。

点击「保存设置」才会写入配置；直接返回会放弃未保存编辑。
加减按钮和恢复原版设置只更新现有输入框，不重新打开窗口。
修改设置只影响新开局，继续已有对局不会重置手牌、出牌、弃牌次数或金钱。
...
在出牌计分、小丑牌触发及回合结算动画中，**按住空格键**即可在当前速度基础上额外加速 **4 倍**，松开立即恢复。例如游戏速度设为 1 倍时变为 4 倍，设为 4 倍时变为 16 倍；原版长结算自带的加速也会保留。

只推进正常动画与事件计时，不跳过小丑牌效果或计分步骤，也不会修改保存的游戏速度。选牌、商店、设置、文字输入、暂停及窗口失去焦点时不启用加速。Windows 和 Linux Proton 使用同一空格键。
...
- `py -3.12 tests/build_qa.py --lovely --fast-forward`：以正常速度、持续按空格、反复按下松开三种方式运行计分场景，核对实际得分、速度恢复和结算耗时。测试驱动模拟按键和窗口焦点，不会安装进正式 Mod。
