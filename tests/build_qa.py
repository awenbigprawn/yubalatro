...
driver = 'ui_layout.lua' if '--ui-layout' in sys.argv else ('scoring.lua' if '--scoring' in sys.argv or '--fast-forward' in sys.argv else 'runtime.lua')
first_case = next((int(a.split('=', 1)[1]) for a in sys.argv if a.startswith('--qa-from=')), 1)
sources['main.lua'] += f'\nYUBALATRO_QA_FROM = {first_case}\n'.encode()
if '--fast-forward' in sys.argv:
    sources['main.lua'] += b'\nYUBALATRO_QA_FAST_FORWARD = true\n'
